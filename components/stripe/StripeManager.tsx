...
import { Loader2, CreditCard, Play, Key } from "lucide-react";
...
export const StripeManager = () => {
    ...
    const fetchConnections = async () => {
        ...
        try {
            ...
            // We reuse the existing endpoint but might need to filter for stripe client-side if the API returns all
            const res = await axios.get('/api/pica/connections', { headers });
            // Filter for stripe
            const stripeConns = res.data.filter((c: Connection) => c.platform === 'stripe');
            setConnections(stripeConns);

            if (stripeConns.length === 0) {
                toast({
                    title: "No Stripe Connections Found",
                    description: "Make sure you have connected Stripe in PicaOS.",
                    variant: "destructive"
                });
            }
        } catch (error: any) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Select a Stripe connection and action to perform.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Select value={selectedConnection} onValueChange={setSelectedConnection}>
                                <SelectTrigger>
                                    <SelectValue placeholder="Select a Stripe connection..." />
                                </SelectTrigger>
                                <SelectContent>
                                    {connections.map((c) => (
